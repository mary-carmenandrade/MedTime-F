...
const NewAppointmentScreen = ({ route }) => {
    ...
    const handleSave = async () => {
        try {
            ...
            navigation.replace('DateDetails', { date }); // Utiliza replace en lugar de navigate
        } catch (e) {
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <ScrollView contentContainerStyle={styles.container}>
                ...
                {isTimeSelected && (
                    <Text style={styles.selectedTimeText}>Hora seleccionada: {hora.toLocaleTimeString()}</Text>
                )}
